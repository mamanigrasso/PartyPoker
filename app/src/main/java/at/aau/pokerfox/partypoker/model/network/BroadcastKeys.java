package at.aau.pokerfox.partypoker.model.network;

public class BroadcastKeys {
    public static final String AMOUNT = "AMOUNT";
    public static final String HAS_FOLDED = "HAS_FOLDED";
    public static final String IS_ALL_IN = "IS_ALL_IN";
    public static final String CARDS = "CARDS";
    public static final String CARD = "CARD";
    public static final String PLAYERS = "PLAYER";
    public static final String CHEAT_ON = "CHEAT_ON";
    public static final String BIG_BLIND = "BIG_BLIND";
    public static final String PLAYER_POT = "PLAYER_POT";
    public static final String IS_SMALL_BLIND = "IS_SMALL_BLIND";
    public static final String IS_BIG_BLIND = "IS_BIG_BLIND";
    public static final String IS_DEALER = "IS_DEALER";
    public static final String MIN_AMOUNT_TO_RAISE = "MIN_AMOUNT_TO_RAISE";
    public static final String NEW_POT = "CARD";
    public static final String WINNER_INFO = "WINNER_INFO";
    public static final String FINAL_WINNER = "FINAL_WINNER";
}
